...
public class LoseWeightFragment extends BaseFragment<Video_QueryFPresenter> implements Contract.IView {
    RecyclerView xrecycVideoLoseweight;
    DanmakuView video_danmu4;
    private int i = 1;
    private Video_QueryBean video_queryBean;
    private String contents;
    private DanmakuContext mContext;
    private AcFunDanmakuParser mParser;
    private static final String TAG = "LoseWeightFragment";
    ...
    protected int initLayout() {
        return R.layout.loseweight_fragment;
    }

    @Override
    protected void initView(View view) {
        xrecycVideoLoseweight = view.findViewById(R.id.xrecyc_video_loseweight);
        video_danmu4 = view.findViewById(R.id.video_danmu4);
    }
    ...
    public void onSuccess(Object data) {
        if (data instanceof Video_QueryBean) {
            ...
            xrecycVideoLoseweight.setAdapter(videoAdapter);
            mlayoutManager = new PagerLayoutManager(getActivity(), OrientationHelper.VERTICAL);
            xrecycVideoLoseweight.setLayoutManager(mlayoutManager);
            ...
            videoAdapter.setOnDian(new VideoAdapter.setOnDian() {
                ...
                public void onPriceClick(int ci) {
                    if (ci%2==1){
                        video_danmu4.setVisibility(View.GONE);
                    }else if (ci%2==0){
                        video_danmu4.setVisibility(View.VISIBLE);
                    }
                }
            });
        }
    ...
    protected void initData() {
        mPresenter.onVideo_Query("4","1","5");
        ...
        video_danmu4.prepare(mParser, mContext);
        video_danmu4.enableDanmakuDrawingCache(true);
        if (video_danmu4 != null) {
            video_danmu4.setCallback(new master.flame.danmaku.controller.DrawHandler.Callback() {
                @Override
    ...
    private void addDanmaku(boolean islive) {
        ...
        if (danmaku == null || video_danmu4 == null) {
            return;
        }
        ...
        danmaku.setTime(video_danmu4.getCurrentTime() + 1200);
        danmaku.textSize = 20f * (mParser.getDisplayer().getDensity() - 0.6f); //文本弹幕字体大小

        video_danmu4.addDanmaku(danmaku);
    }
    @Override
    public void onPause() {
        super.onPause();
        if (video_danmu4 != null && video_danmu4.isPrepared()) {
            video_danmu4.pause();
        }
    }
    ...
    public void onResume() {
        super.onResume();
        if (video_danmu4 != null && video_danmu4.isPrepared() && video_danmu4.isPaused()) {
            video_danmu4.resume();
        }
    }
