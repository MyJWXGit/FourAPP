...
public class FitnessFragment extends BaseFragment<Video_QueryFPresenter> implements Contract.IView {

    RecyclerView xrecycVideoFitness;
    DanmakuView video_danmu3;
    private int i = 1;
    ...
    protected int initLayout() {
        return R.layout.fitness_fragment;
    }

    @Override
    protected void initView(View view) {
        xrecycVideoFitness = view.findViewById(R.id.xrecyc_video_fitness);
        video_danmu3 = view.findViewById(R.id.video_danmu3);
    }

    @Override
    protected void initData() {
        mPresenter.onVideo_Query("3","1","5");
        ...
        video_danmu3.prepare(mParser, mContext);
        video_danmu3.enableDanmakuDrawingCache(true);
        if (video_danmu3 != null) {
            video_danmu3.setCallback(new master.flame.danmaku.controller.DrawHandler.Callback() {
                @Override
    ...
    public void onSuccess(Object data) {
        if (data instanceof Video_QueryBean) {
            ...
            xrecycVideoFitness.setAdapter(videoAdapter);
            mlayoutManager = new PagerLayoutManager(getActivity(), OrientationHelper.VERTICAL);
            xrecycVideoFitness.setLayoutManager(mlayoutManager);
            ...
            videoAdapter.setOnDian(new VideoAdapter.setOnDian() {
                ...
                public void onPriceClick(int ci) {
                    if (ci%2==1){
                        video_danmu3.setVisibility(View.GONE);
                    }else if (ci%2==0){
                        video_danmu3.setVisibility(View.VISIBLE);
                    }
                }
            });
        }
            ...
            Video_Query_BarrageBean video_query_barrageBean = (Video_Query_BarrageBean) data;
             message2 = video_query_barrageBean.getMessage();
    ...
    private void addDanmaku(boolean islive) {
        ...
        if (danmaku == null || video_danmu3 == null) {
            return;
        }
        ...
        danmaku.setTime(video_danmu3.getCurrentTime() + 1200);
        danmaku.textSize = 20f * (mParser.getDisplayer().getDensity() - 0.6f); //文本弹幕字体大小

        video_danmu3.addDanmaku(danmaku);
    }
    @Override
    public void onPause() {
        super.onPause();
        if (video_danmu3 != null && video_danmu3.isPrepared()) {
            video_danmu3.pause();
        }
    }
    ...
    public void onResume() {
        super.onResume();
        if (video_danmu3 != null && video_danmu3.isPrepared() && video_danmu3.isPaused()) {
            video_danmu3.resume();
        }
    }
